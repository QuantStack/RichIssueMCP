...
def validate_required_fields(item: Dict[str, Any]) -> List[str]:
    ...
    required_fields = {
        ...
        "author", "labels", "assignees", "comments", "number_of_comments"
    }
...
def validate_comments_field(item: Dict[str, Any]) -> List[str]:
    ...
    if actual_count != number_of_comments:
        issues.append(f"comment count mismatch: number_of_comments={number_of_comments}, actual={actual_count}")
    
    # Validate comment structure
    required_comment_fields = {"id", "body", "createdAt", "updatedAt", "author", "authorAssociation"}
...
def validate_pr_specific_fields(item: Dict[str, Any]) -> List[str]:
    ...
    url = item.get("url", "")
    has_pr_fields = any(field in item for field in ["mergeable", "merged", "mergedAt", "baseRefName", "headRefName"])
...
def validate_database(repo: str, delete_invalid: bool = False) -> bool:
    ...
    for i, item in enumerate(items[:100]):  # Check first 100 for detailed output
        validation_issues = validate_item(item)
        if validation_issues:
            items_with_detailed_issues.append((item.get("number", f"item_{i}"), validation_issues))
    ...
    if delete_invalid and invalid_items:
        ...
        # Confirm deletion
        confirm = input("\n❓ Are you sure you want to delete invalid items? (yes/no): ").strip().lower()
        if confirm in ('yes', 'y'):
    ...
    health_score = ((total_items - items_with_issues) / total_items * 100) if total_items > 0 else 0
